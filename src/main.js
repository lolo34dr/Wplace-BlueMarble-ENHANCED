...
// ---------- i18n: place this near the top (après stylesheetLink) ----------
const I18N = {
  en: {
    title: name, // garde le nom dynamique
    username: 'Username:',
    droplets: 'Droplets:',
  ...
  fr: {
    ...
    droplets: 'Gouttes :',
    nextlevel: 'Niveau suivant dans...',
    upload_template: 'Téléverser un gabarit',
    enable: 'Activer',
    create: 'Créer',
    disable: 'Désactiver',
    status_placeholder: `Statut : En veille...\nVersion : ${version}`,
    made_by: 'Fait par SwingTheVine - AMÉLIORÉ Par lolo34dr',
    enable_all: 'Tout activer',
    disable_all: 'Tout désactiver',
    no_template_colors: 'Aucune couleur de gabarit à afficher.',
    coords_tlx: 'Tl X',
    coords_tly: 'Tl Y',
    coords_pxx: 'Px X',
    coords_pxy: 'Px Y',
  ...
  jp: {
    ...
    create: '作成',
    disable: '無効化',
    status_placeholder: `ステータス：スリープ中...\nバージョン：${version}`,
    made_by: '作成者 SwingTheVine - 改良者 lolo34dr',
    enable_all: 'すべて有効にする',
    disable_all: 'すべて無効にする',
    no_template_colors: '表示するテンプレート色がありません。',
    coords_tlx: 'Tl X',
    coords_tly: 'Tl Y',
    coords_pxx: 'Px X',
    coords_pxy: 'Px Y',
...
// helper: returns translated string for current lang
function t(key) {
  try {
    const lang = GM_getValue('bmLang', 'fr'); // default FR (tu peux changer)
...
function setLang(lang) {
  GM.setValue('bmLang', lang);
  updateTexts();
  overlayMain.handleDisplayStatus && overlayMain.handleDisplayStatus(t('language_changed'));
}

// create a <select> in the header for language picking
function createLanguageSelector() {
  const header = document.querySelector('#bm-contain-header');
  if (!header) return;

  // avoid double insertion
  if (document.querySelector('#bm-select-lang')) return;
  ...
  Object.keys(I18N).forEach(code => {
    ...
    opt.value = code;
    // small label: EN -> English, FR -> Français, JP -> 日本語
    const label = code === 'en' ? 'English' : code === 'fr' ? 'Français' : code === 'jp' ? '日本語' : code;
    opt.textContent = label;
    select.appendChild(opt);
  });

  const current = GM_getValue('bmLang', 'fr');
...
// update texts already present in the DOM
function updateTexts() {
  try {
    // Header title (the <h1> inside bm-contain-header)
    const h1 = document.querySelector('#bm-contain-header h1');
    if (h1) h1.textContent = t('title');

    // user info
    const uName = document.querySelector('#bm-user-name');
    if (uName) uName.textContent = t('username');
    const uDrops = document.querySelector('#bm-user-droplets');
    if (uDrops) uDrops.textContent = t('droplets');
    const uNext = document.querySelector('#bm-user-nextlevel');
    if (uNext) uNext.textContent = t('nextlevel');

    // coord placeholders
    const tx = document.querySelector('#bm-input-tx');
    if (tx) tx.setAttribute('placeholder', t('coords_tlx'));
    const ty = document.querySelector('#bm-input-ty');
    if (ty) ty.setAttribute('placeholder', t('coords_tly'));
    const px = document.querySelector('#bm-input-px');
    if (px) px.setAttribute('placeholder', t('coords_pxx'));
    const py = document.querySelector('#bm-input-py');
    if (py) py.setAttribute('placeholder', t('coords_pxy'));

    // buttons / labels
    const upload = document.querySelector('#bm-input-file-template');
    if (upload) upload.textContent = t('upload_template');

    const btnEnable = document.querySelector('#bm-button-enable');
    if (btnEnable) btnEnable.textContent = t('enable');
    const btnCreate = document.querySelector('#bm-button-create');
    if (btnCreate) btnCreate.textContent = t('create');
    const btnDisable = document.querySelector('#bm-button-disable');
    if (btnDisable) btnDisable.textContent = t('disable');

    const statusArea = document.getElementById(overlayMain.outputStatusId);
    if (statusArea) statusArea.setAttribute('placeholder', t('status_placeholder'));

    const madeBy = document.querySelector('#bm-contain-buttons-action small');
    if (madeBy) madeBy.textContent = t('made_by');

    // color filter header/buttons
    const btnColorsEnableAll = document.querySelector('#bm-button-colors-enable-all');
    if (btnColorsEnableAll) btnColorsEnableAll.textContent = t('enable_all');
    const btnColorsDisableAll = document.querySelector('#bm-button-colors-disable-all');
    if (btnColorsDisableAll) btnColorsDisableAll.textContent = t('disable_all');

    // color list empty text
    const listContainer = document.querySelector('#bm-colorfilter-list');
    if (listContainer && (!templateManager.templatesArray?.[0] || !templateManager.templatesArray[0].colorPalette)) {
      listContainer.innerHTML = `<small>${t('no_template_colors')}</small>`;
    }

    // update any dynamic values (if you use data-i18n in future)
    document.querySelectorAll('[data-i18n]').forEach(el => {
      const key = el.getAttribute('data-i18n');
      el.textContent = t(key);
    });
  } catch (e) {
    console.warn('i18n: failed to update texts', e);
  }
}

// call these after overlay est construit
// (si buildOverlayMain() a déjà été appelé plus haut, appelle ces fonctions juste après)
